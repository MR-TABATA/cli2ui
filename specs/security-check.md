...
実施日: 2026-06-11（初回） / **2026-06-13 再実行**（i18n・公開準備の差分込み。`views.py` 分割後 = `core/views/` パッケージ、`set_language`/`LocaleMiddleware` 追加後）
対象: `core` / `cli2ui`（Django バックエンド。テスト・マイグレーション除く）
結果: **要対応の指摘はすべて是正済み**（bandit Low/Medium/High = 0 / pip-audit runtime = 0 / `check --deploy` は HTTPS 環境専用の警告 3 件のみ＝ローカル HTTP では受容）。2026-06-13 再実行でも同判定（新規 Low 1 件 = best-effort ログの try/except/pass を理由付き `# nosec B110` で抑制）。
