...
## いちばん綺麗: cli2ui を自分のプロジェクトの compose に同居させる

データベースが既に自分の管理する `docker compose` プロジェクトにいるなら、いちばん
綺麗なのは **cli2ui をその同じ compose にサービスとして足す**ことです（別スタック
...
```bash
docker compose --profile tools up    # 自分のスタック + cli2ui を起動
```

`http://localhost:8001` を開き、**host = 自分の db のサービス名**（例 `db`）・port
`5432`・自分の資格情報で接続します。

外さないための3点：

1. **`profiles` で開発専用に隔離する。** cli2ui に認証はありません。`profiles:
   ["tools"]` を付けると、ただの `docker compose up` では起動せず、`--profile
   tools` を付けたときだけ立ち上がります。これが「共有環境や本番スタックにうっかり
   同居する」ことへの保険です。
2. **cli2ui 同梱の `sampledb` は持ち込まない。** 向けたいのは*自分の*データベース
   （サービス名で指定）であってデモDBではありません。上の `cli2ui` サービス1つで
   十分で、ビルド済みの `cli2ui-app` イメージを参照すれば cli2ui 自身の compose や
   build context から切り離せます。
3. **空きのホストポートを使う**（ここでは `8001:8000`）——アプリと衝突しないように。

> **保存した接続・履歴**はコンテナ内の `db.sqlite3` にあるので、コンテナが作り直さ
> れるとリセットされます——開発ツールとしては許容範囲（接続の入れ直しは数秒）。残し
> たいなら、ファイルを bind mount してください（事前に作成した
> `./.cli2ui-db.sqlite3:/app/db.sqlite3`）。
